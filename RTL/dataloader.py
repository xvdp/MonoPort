...
class _MultiProcessingDataLoaderIter(_BaseDataLoaderIter):
    ...
    def __init__(self, loader):
        ...
        if self._pin_memory:
            ...
            self._pin_memory_result_queue = queue.Queue()
            pin_memory_thread = threading.Thread(
                target=_utils.pin_memory._pin_memory_loop,
                args=(self._worker_result_queue, self._pin_memory_result_queue,
        ...
        #     self._post_process_thread_done_event = threading.Event()
        #     self._data_queue = queue.Queue()
        #     post_process_thread = threading.Thread(
        #         target=_postprocess_loop,
        #         args=(self._pin_memory_result_queue, self._data_queue,
        #               torch.cuda.current_device(),
        #               self._post_process_thread_done_event, lambda x: x.cuda()))
        #     post_process_thread.daemon = True
        #     post_process_thread.start()
        #     # Similar to workers (see comment above), we only register
        #     # post_process_thread once it is started.
        #     self._post_process_thread = post_process_thread
        # else:
        #     self._data_queue = self._pin_memory_result_queue
    ...
    def _get_data(self):
        ...
        if self._timeout > 0:
            ...
            while self._pin_memory_thread.is_alive(): # TODO
    ...
    def _shutdown_workers(self):
        ...
        if not self._shutdown:
            ...
            try:
                ...
                #     self._post_process_thread_done_event.set()
                #     # Send something to _post_process_thread in case it is waiting
                #     # so that it can wake up and check `_post_process_thread_done_event`
                #     self._pin_memory_result_queue.put((None, None))
                #     self._post_process_thread.join()
